define([
	...
	Context.prototype.add = function(config) {
		...
		function _flushObservers() {
			for (var i = 0, l = facade.observers.length; i < l; i++) {
				facade.observers[i]();
			}
		}
	...
	Context.prototype.parse = function(container, setter) {
		var that			= this,
			...
			involvedProps	= false,
			model			= this.model,
			path, children, expr, key, prop, current, i, l;

		path = typeof setter === 'string' && setter !== '' ? setter : '/';
