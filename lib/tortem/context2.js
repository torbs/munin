define([
	...
	Context.prototype.add = function(config) {
		...
		setter = new Setter(current.match, dStore);
		...
		facade.props = facade.props.concat( config.involvedProps ? config.involvedProps : [config.property]);
	...
	Context.prototype.parse = function(container, setter) {
		var that			= this,
			...
			model			= this.model,
			path			= '/',
			children, expr, key, prop, current, i, l, tempEl, remove;
